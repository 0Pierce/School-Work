...
//-----------------------------------------------------------------------------------------------
		...
		lblLightRain.setIcon(new ImageIcon("GIFS/LightRain.gif"));
		...
		lblTaxi.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent me) {
				...
				//All of this ensures that players can only play the specified amount of times (Mr.X twice if he picks double card)
				if ((b == 0 && Ticket.detectiveStock[0][0] >= 1 && count >= 2) && (helper.getValue()/100%10 == 1 || helper.getValue() == 1)) {
					...
					//Plays the sound
					File sound = new File("sounds/CardPick.wav");
					try {
					...
					Ticket.setMrXTurn(false);
					Ticket.useTaxi();

					//Flag which helps ensure that players cant play more then they should
					count--;

					//Updates the detective taxi lbl
					lblTaxiDAmount.setText("" + Ticket.detectiveStock[0][0]);
					makeMove();
					
					
				} else if (b == 0 && Ticket.detectiveStock[0][0] < 1) {
					lblTaxiDAmount.setText("None");

				}

				//B == 1 (Mr.X)
				if ((b == 1 && XTaxiAmount >= 1 && count == 3) &&  (helper.getValue()/100%10 == 1 || helper.getValue() == 1)) {
					...
					Ticket.setMrXTurn(true);
					Ticket.useTaxi();;

					
					lblTaxiDAmount.setText("" + Ticket.detectiveStock[1][0]);
...
//----------------------------------------------------------------------------------------------------------------
		...
		lblBus.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent me) {
				...
				if ((b == 0 && Ticket.detectiveStock[1][0] > 1 && count >= 2) && (helper.getValue()/100%10 == 1|| helper.getValue() == 2)) {
					...
					lblBusDAmount.setText("" + Ticket.detectiveStock[1][0]);
					// lblBusXAmount.setText("" + XBusAmount);
				} else if (b == 0 && Ticket.detectiveStock[1][0] < 1) {
					lblBusDAmount.setText("None");

				}

				if ((b == 1 && XBusAmount > 1 && count >= 3) && (helper.getValue()/100%10 == 1|| helper.getValue() == 2)) {
...
//---------------------------------------------------------------------------------------------------------------------
		...
		lblTube.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent me) {
				...
				if ((b == 0 && Ticket.detectiveStock[2][0] >= 1 && count >= 2) && (helper.getValue()/1000%10 == 1|| helper.getValue() == 3)) {
					...
					lblTubeDAmount.setText("" + Ticket.detectiveStock[2][0]);
				...
				if ((b == 1 && XTubeAmount > 1 && count >= 3 && count <= 5) && (helper.getValue()/1000%10 == 1|| helper.getValue() == 3)) {
		...
		lbBlackTicket.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent me) {
				...
				if (b == 1 && Ticket.mrXStock[0][0] > 1 && count == 3) {
					...
					Ticket.setMrXTurn(true);
					Ticket.useBlackTicket();
					
					lbBlackTicketAmount.setText("" + Ticket.mrXStock[0][0]);
					int c = 4;
					turncount++;
					count = 0;
		...
		X2TicketIcon.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent me) {
				...
				//Otherwise its the same as the first label
				if (b == 1 && Ticket.mrXStock[1][0] > 1 && count == 3) {
					...
					Ticket.setMrXTurn(true);
					Ticket.use2xTicket();
					X2Amount.setText("" + Ticket.mrXStock[1][0]);
...
//------------------------------------------------------------------------------------
	...
	//Just a simple method that sends the card information to the travellog
